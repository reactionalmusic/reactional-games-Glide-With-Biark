...
public class CameraShake : MonoBehaviour
{
    ...
    private bool isShaking; // FLag to sheck if shake is already happening
    ...
    ///     This method ensures that only one shake effect occurs at a time by checking the `isShaking` flag before starting a
    ///     new shake.
    /// </summary>
    /// <param name="duration">
    ///     The total duration of the shake effect in seconds. This controls how long the shake effect will
    ///     last.
    /// </param>
    /// <param name="startAmplitude">
    ///     The starting amplitude of the shake. This controls the initial intensity of the camera's
    ///     shake effect.
    /// </param>
    /// <param name="startFrequency">
    ///     The starting frequency of the shake. This controls the initial speed of the shake
    ///     oscillations.
    /// </param>
    /// <param name="decreeasSpeed">
    ///     A multiplier for the rate at which the shake's magnitude and frequency decrease. A higher
    ///     value makes the shake taper off more quickly, whereas a lower value makes it decrease more slowly. This does not
    ///     affect the total duration of the shake, but rather how quickly it diminishes.
    /// </param>
    ...
    private IEnumerator ShakeDuration(float duration, float startAmplitude, float startFrequency, float decreaseSpeed)
    {
        ...
        while (elapsed < duration)
        {
            ...
            var lerpFactor = elapsed / duration * decreaseSpeed;
            channelPerlinNoise.AmplitudeGain = Mathf.Lerp(startAmplitude, 0f, lerpFactor);
            channelPerlinNoise.FrequencyGain = Mathf.Lerp(startFrequency, 0f, lerpFactor);
            // Debug.Log("Time: " + elapsed + " And the amp: " + channelPerlinNoise.AmplitudeGain + " and freq: " + channelPerlinNoise.FrequencyGain);
            yield return null;
        }
    ...
    {
        channelPerlinNoise.AmplitudeGain = 0f;
        channelPerlinNoise.FrequencyGain = 0f;
    }
}